...
public class ScreenFader : MonoBehaviour
{
    ...
    public void FadeTransition(Action afterFade)
    {
        if (!m_IsFading)
            StartCoroutine(FadeInOut(afterFade));
    }

    public void FadeTransition()
    {
        if (!m_IsFading)
            StartCoroutine(FadeInOut(null));
    }
    ...
    private IEnumerator FadeInOut(Action afterFade)
    {
        m_IsFading = true;
        canvasGroup.blocksRaycasts = true;
        ...
        if (canvasGroup.alpha == 0)
        {
            ...
            {
                canvasGroup.alpha = Mathf.MoveTowards(canvasGroup.alpha, 1, m_FadeSpeed * Time.deltaTime);
                yield return null;
            }
            canvasGroup.alpha = 1;
        }
        ...
        if (canvasGroup.alpha == 1)
        {
            ...
            {
                canvasGroup.alpha = Mathf.MoveTowards(canvasGroup.alpha, 0, m_FadeSpeed * Time.deltaTime);
                yield return null;
            }
            canvasGroup.alpha = 0;
        }
